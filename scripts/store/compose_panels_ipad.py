...
for name, l1, l2, sub in PANELS:
    ...
    c.alpha_composite(TRACK_ART, (-300, -120))
    glow_blob(c, (W // 2, 1200), 900, BLUE, 46)
    glow_blob(c, (W - 110, 300), 330, RED, 36)
    glow_blob(c, (90, 2700), 380, BLUE, 30)

    headline(c, l1, l2, sub)

    shot = Image.open(f"{RAW}/ipad-{name}.png").convert("RGB")
    dev = device(shot)
    dx = (W - dev.width) // 2
    dy = 940
    ...
    c.convert("RGB").save(f"{PROJ}/fastlane/screenshots/ipad/0{PANELS.index((name, l1, l2, sub)) + 1}-{name}.png", quality=95)
    print("v3", name)

sheet = Image.new("RGB", (6 * 440 + 60, 1000), (8, 10, 16))
for i, (name, *_r) in enumerate(PANELS):
    p = Image.open(f"{PROJ}/fastlane/screenshots/ipad/0{i + 1}-{name}.png").resize((440, 956), Image.LANCZOS)
    sheet.paste(p, (10 + i * 440, 22))
sheet.save(f"{PROJ}/Marketing/store-ipad-sheet.png")
print("sheet done")
